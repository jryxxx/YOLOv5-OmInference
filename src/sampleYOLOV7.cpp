...
Result SampleYOLOV7::Inference(std::vector<InferenceOutput>& inferOutputs)
{
    ...
    std::cout << "Debug -> Input's size is " << resizedImage_.size << std::endl;
    if (ret == FAILED) {
                               ...
                               string imagePath, size_t imageIndex, bool release)
{
    ...
    std::cout << "Debug -> Output's size is " << inferOutputs[outputDataBufId].size << std::endl;
...
int main()
{
    ...
    std::cout << "Inference time is -> " << duration.count() << "ms" << std::endl;
    return SUCCESS;
}
